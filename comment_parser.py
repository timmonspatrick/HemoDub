...
def exponent(n):
    '''An incredibly lazy exponent parsing function'''
    if n.split("x")[1] == "103":
...
def hemolytik_parser(comment, mw):
    ...
    if hemolytic == True:      ##Binary state stored in string 
        return "YES"           ##to allow future expansion with MAYBE
    elif hemolytic == False:   ##Also, easier to store string in a JSON
        return "NO"            ##And reparse later
