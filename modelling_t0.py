...
inp = np.load("inputs/peptides_array_aahydrophobicity.npy", allow_pickle=False)
...
for train, test in kfold.split(X_ann, Y):
    ...
    model_ann.add(Dense(32, input_shape=(dims,), init='uniform', activation="relu"))
    model_ann.add(Dropout(dropout, noise_shape=None, seed=42))
    #model_ann.add(Dense(4,  init='uniform', activation='relu'))
    #model_ann.add(Dropout(dropout, noise_shape=None, seed=42))
    #model_ann.add(Dense(4,  init='uniform', activation='relu'))
    ...
    model_ann.fit(x = X_ann[train], 
              y = Y[train], 
              validation_data = (
                      X_ann[test], 
                      Y[test],
                      ), 
              epochs=200, batch_size=202, verbose=True,
              #shuffle=True, 
              callbacks=[
                      ##best_model, 
                      early_stop,
