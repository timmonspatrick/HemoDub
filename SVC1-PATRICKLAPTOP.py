...
inp = np.load("peptides_array3.npy", allow_pickle=False)
...
X_train, X_val, Y_train, Y_val = train_test_split(X_train_org, Y_train_org, test_size=0.20, random_state=31)

Y_train = [i[0] for i in Y_train]
Y_val = [i[0] for i in Y_val]

model = SVC(kernel='linear', C=1E10)
#model = LinearSVC(random_state=0, C=0.01)
