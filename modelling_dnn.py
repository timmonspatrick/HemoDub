...
from keras.layers import Dense, Activation, Dropout, Conv1D, Embedding, MaxPooling1D, LSTM, Merge
...
for train, test in kfold.split(X_ann, Y):
    ...
    models = []
    for embed_vocab, X_rnn, inp_len, kernel_size in [(embed_vocab_0, X_rnn_0, 200, 16), (embed_vocab_1, X_rnn_1, 15, 4), (embed_vocab_2, X_rnn_2, 15, 4), (embed_vocab_3, X_rnn_3, 15, 4), (embed_vocab_4, X_rnn_4, 15, 4), ]:
        print(inp_len)
        ...
        model_rnn.add(MaxPooling1D(pool_size=5, strides=5))
        model_rnn.add(LSTM(100, unroll = True, stateful = False, dropout = 0.25))
        #model_rnn.add(Dense(1, init='normal', activation="sigmoid"))
        models.append(model_rnn)
        
    model_ann = Sequential()
    #model.add(Dense(1024, input_shape=(dims,), init='uniform', activation='relu'))
    model_ann.add(Dropout(0.7, input_shape=(dims_ann,), noise_shape=None, seed=42))
    model_ann.add(Dense(64,  init='normal', activation='relu'))
    model_ann.add(Dense(32,  init='normal', activation='relu'))
    model_ann.add(Dropout(0.5, noise_shape=None, seed=42))
    #model_ann.add(Dense(1, init='normal', activation="sigmoid"))
    models.append(model_ann)
    
    model = Sequential()
    model.add(Merge(models, mode="concat"))
    model.add(Dropout(0.5, seed=42, noise_shape=None))
    ...
    model.fit(x = [X_rnn_0[train], X_rnn_1[train], X_rnn_2[train], X_rnn_3[train], X_rnn_4[train], X_ann[train]], 
              y = Y[train], 
              validation_data = (
                      [X_rnn_0[test], X_rnn_1[test], X_rnn_2[test], X_rnn_3[test], X_rnn_4[test], X_ann[test]], 
                      Y[test],
                      ), 
              epochs=100, batch_size=dims, verbose=True,
              #shuffle=True, 
              callbacks=[
                      ##best_model, 
                      early_stop,
    ...
    score = model.evaluate([x_test_rnn, x_test_ann], Y[test], batch_size=32, verbose=1)
