...
from keras.layers import Dense, Activation, Dropout, Conv1D, Embedding, MaxPooling1D, LSTM, Merge, GRU, Bidirectional, Flatten
...
for train, test in kfold.split(X_ann, Y):
    ...
    model_ann.add(Dense(256, input_shape=(dims,), init='uniform', activation='relu'))
    #model_ann.add(Dropout(0.25, noise_shape=None, seed=42))
    model_ann.add(Dense(256,  init='uniform', activation='relu'))
    #model_ann.add(Dropout(0.25, noise_shape=None, seed=42))
    model_ann.add(Dense(256,  init='uniform', activation='relu'))
    #model_ann.add(Dropout(0.25, seed=42, noise_shape=None))
    models.append(model_ann)
        
    model = Sequential()
    model.add(Merge(models, mode="concat"))
    model.add(Dense(16, init="normal", activation="relu"))
    #model.add(Dropout(0.63, seed=42, noise_shape=None))
    ...
    model.compile(optimizer=adagrad, loss='categorical_hinge', metrics=["binary_accuracy"])
    model.summary()
    ...
    tensorboard_callback = TensorBoard(log_dir='./tb_logs_020501', histogram_freq=0,  
          write_graph=True, write_images=True)
    
    model.fit(x = [#X_rnn_0[train], X_rnn_1[train], X_rnn_2[train], X_rnn_3[train], X_rnn_4[train], 
                   X_rnn_aai_x[train], X_ann[train]], 
              y = Y[train], 
              validation_data = (
                      [#X_rnn_0[test], X_rnn_1[test], X_rnn_2[test], X_rnn_3[test], X_rnn_4[test],  
                      X_rnn_aai_x[test], X_ann[test]], 
                      Y[test],
                      ), 
    ...
    cvsscores.append(model.evaluate(x=[ X_rnn_aai_x[test], X_ann[test]], y=Y[test]))
