...
def counter_1(string):
    l = len(string)
    d = {i : 0 for i in letters[1]}
    for s in string:
        try:
            d[s] += 1.0
        except KeyError:
...
def counter_3(string):
    l = len(string)
    d = {i : 0 for i in letters[3]}
    for a in range(l-2):
        s = string[a:a+3]
        try:
            d[s] += 1.0
...
def counter_4(string):
    l = len(string)
    d = {i : 0 for i in letters[4]}
    for s in string:
        try:
            d[s] += 1.0
        except KeyError:
...
def residue_distribution(all_residues, seq_type, dp, tp):
    '''
